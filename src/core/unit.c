...
static int user_from_unit_name_with_instance(Unit *u, char **ret) {
        ...
        r = unit_name_to_prefix_and_instance(u->id, &n);
        if (r < 0)
                return r;


        if (valid_user_group_name(n, VALID_USER_RELAX|VALID_USER_WARN)) {
                *ret = TAKE_PTR(n);
                return 0;
        }
        ...
        log_unit_notice(u, "Fetched username by instance : %s, using %s", n, *ret);
...
int unit_patch_contexts(Unit *u) {
        ...
        if (ec) {
                ...
                if (ec->dynamic_user) {
                        if (!ec->user) {
                                if (ec->dynamic_user_by_instance)
                                        r = user_from_unit_name_with_instance(u, &ec->user);
                                else
